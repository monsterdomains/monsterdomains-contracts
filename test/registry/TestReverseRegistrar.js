...
contract('ReverseRegistar', function (accounts) {
    ...
    let node;
    let registrar, resolver, mid, nameWrapper;
    ...
    beforeEach(async () => {
        ...
        registrar = await ReverseRegistrar.new(mid.address, resolver.address);

        await mid.setSubnodeOwner('0x0', sha3('reverse'), accounts[0], {from: accounts[0]});
        await mid.setSubnodeOwner(namehash.hash('reverse'), sha3('addr'), registrar.address, {from: accounts[0]});
    });
    ...
    it('allows an account to specify resolver', async () =>  {
        ...
        assert.equal(await mid.owner(node), accounts[1]);
        assert.equal(await mid.resolver(node), accounts[2]);
    });
    ...
    it('does not overwrite resolver if not specified', async () => {
        ...
        assert.equal(await mid.owner(node), accounts[3]);
        assert.equal(await mid.resolver(node), accounts[2]);
    });

    it('sets name records', async () => {
        await registrar.setName('testname', {from: accounts[0], gas: 1000000});
        assert.equal(await mid.resolver(node), resolver.address);

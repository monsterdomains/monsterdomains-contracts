...
const { evm, exceptions, address } = require("../test-utils");
...
contract('MIDRegistrarController', function (accounts) {
    let mid;
    let resolver;
    let baseRegistrar;
    let controller;
    ...
    before(async () => {
        ...
        priceOracle = await StablePriceOracle.new(dummyOracle.address, [1, 1, 1, 1, 1]);
        ...
        resolver = await PublicResolver.new(mid.address, nameWrapper.address, address.NULL_ADDRESS, address.NULL_ADDRESS);
    });
    ...
    it('should permit new registrations with config', async () => {
        ...
        var nodehash = namehash.hash("newconfigname.bnb");
        assert.equal((await mid.resolver(nodehash)), resolver.address);
        assert.equal((await mid.owner(nodehash)), registrantAccount);
    ...
    it('should not allow a commitment with addr but not resolver', async () => {
        await exceptions.expectFailure(controller.makeCommitmentWithConfig("newconfigname2", registrantAccount, secret, address.NULL_ADDRESS, registrantAccount));
    });

    it('should permit a registration with resolver but not addr', async () => {
        var commitment = await controller.makeCommitmentWithConfig("newconfigname2", registrantAccount, secret, resolver.address, address.NULL_ADDRESS);
        var tx = await controller.commit(commitment);
        assert.equal(await controller.commitments(commitment), (await web3.eth.getBlock(tx.receipt.blockNumber)).timestamp);

        await evm.advanceTime((await controller.minCommitmentAge()).toNumber());
        var balanceBefore = await web3.eth.getBalance(controller.address);
        var tx = await controller.registerWithConfig("newconfigname2", registrantAccount, 28 * DAYS, secret, resolver.address, address.NULL_ADDRESS, {value: 28 * DAYS + 1});
        assert.equal(tx.logs.length, 1);
        assert.equal(tx.logs[0].event, "NameRegistered");
        ...
        var nodehash = namehash.hash("newconfigname2.bnb");
        assert.equal((await mid.resolver(nodehash)), resolver.address);
